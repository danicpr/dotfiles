...
[User Verification Query] ➔ [Spawn Blind QA Verifier Subagent]
  ...
  ├── (100% Pass) ➔ [Master Delivers Final Summary to User]
...
5. **Final Delivery**: Master compiles findings and presents the final report to the user in their requested language.
...
- **Async Yield**: Stop calling tools immediately after `invoke_subagent`. Await reactive wakeup. Never poll.
...
- **Final Delivery**: Compile verified logs, diff summaries, and test evidence into a complete, professional report delivered to the user.

---

## 5. Async Yielding & Interaction Protocol
...
- **User Delivery**: Professional, clear communication mirroring the user's preferred language and tone for all user interactions.
